    ...
    try {
        ...
        if (err.line) {
            ...
            while(i > 0 && !err.filename) {
              var line = lines[i];
              if(line.substr(0,21) == '/*** uncss> filename:') {
                err.filename = line.substring(22,line.length - 4);
                zeroLine = i;
              }
              i--;
            }

            for(var i = err.line - 6; i < err.line + 5; i++) {
              if(i - zeroLine < 0) continue;
              var line = lines[i];
