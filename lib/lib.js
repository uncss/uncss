 ...
 */
function filterUnusedSelectors(doms, selectors) {
    ...
    return selectors.filter(function (selector) {
        ...
        for (i = 0; i < doms.length; ++i) {
            ...
            try {
                ...
                /* Remove ':' pseudos, but keep those inside quoted strings.
 ...
 */
function filterUnusedRules(doms, stylesheet) {
    ...
    rules.forEach(function (rule) {
        if (rule.type === 'rule') {
            rule.selectors = filterUnusedSelectors(doms, rule.selectors);
        } else if (rule.type === 'media') {
            rule.rules = filterUnusedRules(doms, { rules: rule.rules }).stylesheet.rules;
        }
    });
