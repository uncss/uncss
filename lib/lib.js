...
function readStylesheets(files, callback) {
    return async.map(files, function (filename, done) {
        if (filename.match(/^http/)) {
            ...
            request(filename, { headers: {'User-Agent': 'UnCSS', 'Accept-Encoding':'gzip'} }).on('response', function (resp) {
              if (resp.statusCode !== 200) { return done(new Error('Status not 200')); }
              if (resp.headers['content-encoding'] && resp.headers['content-encoding'].indexOf('gzip') !== -1) resp = resp.pipe(zlib.createGunzip());
              var body = "";
